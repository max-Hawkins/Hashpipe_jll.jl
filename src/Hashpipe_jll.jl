...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import Hashpipe_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("Hashpipe_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "Hashpipe_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"Hashpipe", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "Hashpipe_jll", "src", "Hashpipe_jll.jl"))
        @info("Hashpipe_ll dev'ed out to /home/mhawkins/.julia/dev/Hashpipe_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("a3e50fc0-e135-5cb8-92f4-a3765d91b533"))
platforms = [Pkg.Artifacts.unpack_platform(e, "Hashpipe", artifacts_toml) for e in artifacts["Hashpipe"]]
...
if best_platform === nothing
    @debug("Unable to load Hashpipe; unsupported platform $(triplet(platform_key_abi()))")
else
